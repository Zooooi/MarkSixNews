...
本系統支援**精確排程、平台定向投放、冷卻時間控制、每日上限、今天不再提示、強制公告鎖定（維護/強更通知）以及點擊導頁跳轉動作**。
...
* **強大的頻率控制**：
  ...
  * 每日最高顯示次數 (`maxCountPerDay`)。
* **[新] 跨平台定向控制 (`platform`)**：支援針對不同作業系統（Android / iOS）進行公告的分離投放，避免 Android 專屬更新或 iOS 專屬活動打擾到非目標平台用戶。
* **使用者體驗優化**：支援「今天不再顯示」勾選框，勾選後當天自動攔截不再打擾用戶。
* **強制公告 (Force Mode)**：當啟用 `force: true` 時，彈窗會隱藏關閉按鈕、防鎖死點擊外部空白處關閉，並使用新版 `PopScope` 攔截 Android 實體返回鍵。適合用作維護或強制更新公告。
* **導頁動作 (Action System)**：公告不只是觀看，還可以設定點擊跳轉到 App 內部指定路由頁面（如儲值頁、活動頁）或透過外部瀏覽器開啟。
...
```json
{
  "news_list": [
    ...
    },
    {
      ...
      "action_value": "[https://google.com](https://google.com)",
      "policy": {
        "enable": "true",
        "release": "false",
        "platform": "a",
